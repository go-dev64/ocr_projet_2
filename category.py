...
def links_all_pages(url, reponse):
    """boucle sur toutes les pages disponibles pour la categorie
    ou sur le site et retourne un liste des liens des livres.
    Args:
    ...
    while reponse.ok:
        ...
        links_of_books_pages.extend(
            get_links_of_page(
                utile.download_book_page(url_var)[0]
                )
            )
...
def get_all_links_of_all_pages(url):
    ...
    if reponse.ok:
        """verifie si plusieurs pages sont disponible,
        et les parcours pour retourner les liens des livres de chaque pages"""
        all_books_url.extend(
            links_all_pages(
                url=url_modifie,
        ...
        """verifie si il y a des liens de livres sur la page
        et retournent tous les liens de cette page. """
        all_books_url.extend(
            get_links_of_page(soupe=utile.download_book_page(url)[0]))
...
def category():
    soup = utile.download_book_page(
        "http://books.toscrape.com/catalogue/category/books_1/index.html")[0]
